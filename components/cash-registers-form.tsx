...
export function CashRegistersForm({
  initial,
}: {
  ...
  function saveNew() {
    ...
    start(async () => {
      const res = await fetch("/api/settings/registers", {
        method: "POST",
      ...
      const data = (await res.json()) as { register: CashRegister };
  ...
  function saveEdit() {
    ...
    const id = editingId;
    start(async () => {
      const res = await fetch(`/api/settings/registers/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  function remove(r: CashRegister) {
    ...
    start(async () => {
      const res = await fetch(`/api/settings/registers/${r.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = (await res.json().catch(() => ({}))) as {
